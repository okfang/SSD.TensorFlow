...
class SE_SSD300_NET(object):
    def __init__(self, data_format='channels_first', backbone_batch_normal=False, additional_batch_normal=False,
        ...
        # VGG layers
        self._conv1_block = self.conv_block(2, 64, 3, (1, 1), 'conv1', trainable=self.is_trainable[
            'conv1']) if not backbone_batch_normal else self.conv_bn_block(2, 64, 3, (1, 1), 'conv1_bn')
        self._pool1 = tf.layers.MaxPooling2D(2, 2, padding='same', data_format=self._data_format, name='pool1')
        self._conv2_block = self.conv_block(2, 128, 3, (1, 1), 'conv2', trainable=self.is_trainable[
            'conv2']) if not backbone_batch_normal else self.conv_bn_block(2, 128, 3, (1, 1), 'conv2_bn')
        self._pool2 = tf.layers.MaxPooling2D(2, 2, padding='same', data_format=self._data_format, name='pool2')
        self._conv3_block = self.conv_block(3, 256, 3, (1, 1), 'conv3', trainable=self.is_trainable[
            'conv3']) if not backbone_batch_normal else self.conv_bn_block(3, 256, 3, (1, 1), 'conv3_bn')
        self._pool3 = tf.layers.MaxPooling2D(2, 2, padding='same', data_format=self._data_format, name='pool3')
        self._conv4_block = self.conv_block(3, 512, 3, (1, 1), 'conv4', trainable=self.is_trainable[
            'conv4']) if not backbone_batch_normal else self.conv_bn_block(3, 512, 3, (1, 1), 'conv4_bn')
        self._pool4 = tf.layers.MaxPooling2D(2, 2, padding='same', data_format=self._data_format, name='pool4')
        self._conv5_block = self.conv_block(3, 512, 3, (1, 1), 'conv5', trainable=self.is_trainable[
            'conv5']) if not backbone_batch_normal else self.conv_bn_block(3, 512, 3, (1, 1), 'conv5_bn')
        ...
        self._conv6 = [tf.layers.Conv2D(filters=1024, kernel_size=3, strides=1, padding='same', dilation_rate=6,
                                        ...
                                        name='fc6', _scope='fc6', _reuse=None, trainable=self.is_trainable[
                'fc6'])] if not additional_batch_normal else self.conv_bn_block(1, 1024, 3, (1, 1), 'fc6_bn')
        self._conv7 = [tf.layers.Conv2D(filters=1024, kernel_size=1, strides=1, padding='same',
                                        data_format=self._data_format, activation=tf.nn.relu, use_bias=True,
                                        kernel_initializer=self._conv_initializer(),
                                        bias_initializer=tf.zeros_initializer(),
                                        name='fc7', _scope='fc7', _reuse=None, trainable=self.is_trainable[
                'fc7'])] if not additional_batch_normal else self.conv_bn_block(1, 1024, 1, (1, 1), 'fc7_bn')
        # SSD layers
        with tf.variable_scope('additional_layers') as scope:
            self._conv8_block = self.ssd_conv_block(256, 2, 'conv8', trainable=self.is_trainable[
                'conv8']) if not additional_batch_normal else self.ssd_conv_bn_block(256, 2, 'conv8_bn')
            self._conv9_block = self.ssd_conv_block(128, 2, 'conv9', trainable=self.is_trainable[
                'conv9']) if not additional_batch_normal else self.ssd_conv_bn_block(128, 2, 'conv9_bn')
            self._conv10_block = self.ssd_conv_block(128, 1, 'conv10', padding='valid', trainable=self.is_trainable[
                'conv10']) if not additional_batch_normal else self.ssd_conv_bn_block(128, 1, 'conv10_bn',
                                                                                      padding='valid')
            self._conv11_block = self.ssd_conv_block(128, 1, 'conv11', padding='valid', trainable=self.is_trainable[
                'conv11']) if not additional_batch_normal else self.ssd_conv_bn_block(128, 1, 'conv11_bn',
                                                                                      padding='valid')
