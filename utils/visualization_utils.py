...
def draw_side_by_side_evaluation_image(eval_dict,
  ...
  if len(eval_dict['detection_classes'].shape) == 1:
    for key in eval_dict:
      if key != 'original_image':
        eval_dict[key] = tf.expand_dims(eval_dict[key], 0)

  for indx in range(eval_dict['original_image'].shape[0]):

    images_with_detections = draw_bounding_boxes_on_image_tensors(
        tf.expand_dims(
            eval_dict['original_image'][indx], axis=0),
        tf.expand_dims(
            eval_dict['detection_boxes'][indx], axis=0),
        tf.expand_dims(
            eval_dict['detection_classes'][indx], axis=0),
        tf.expand_dims(
            eval_dict['detection_scores'][indx], axis=0),
        category_index,
        original_image_spatial_shape=tf.expand_dims(
            eval_dict['original_image_spatial_shape'][indx],
            axis=0),
    ...
    images_with_groundtruth = draw_bounding_boxes_on_image_tensors(
        tf.expand_dims(
            eval_dict['original_image'][indx], axis=0),
        tf.expand_dims(
            eval_dict['groundtruth_boxes'][indx], axis=0),
        tf.expand_dims(
            eval_dict['groundtruth_classes'][indx], axis=0),
        tf.expand_dims(
            tf.ones_like(
                eval_dict['groundtruth_classes'][indx],
                dtype=tf.float32),
            axis=0),
        category_index,
        original_image_spatial_shape=tf.expand_dims(
